...
Penyematan dokumentasi resmi Pantau (README, User Guide dwibahasa ID/EN, dan CHANGELOG) ke dalam single binary via `go:embed`, penyajian dokumen melalui endpoint publik `/api/docs`, dan penampil antarmuka berbasis Workspace Modal dengan client-side micro-parser tanpa dependensi eksternal.

## Konteks dan Masalah

Pantau merupakan sistem pemantauan dan manajemen server mandiri (*self-hosted*) yang beroperasi di lingkungan terisolasi (*airgapped*) atau intranet privat ([ADR 0002](0002-golang-single-binary-sqlite.md) dan [ADR 0013](0013-airgapped-self-contained-web-assets.md)). Repositori Pantau telah dilengkapi dengan dokumentasi menyeluruh (`README.md`, `README.id.md`, `docs/user-guide.md`, `docs/user-guide.id.md`, dan `CHANGELOG.md`).
...
1. **Kendala Airgapped & Bootstrap**: Operator yang menjalankan Pantau di jaringan tanpa akses internet tidak dapat membuka repositori online untuk membaca panduan setup, restore, konfigurasi Desired State, atau riwayat perubahan versi (Changelog).
...
1. **Embedded Single-Source-of-Truth (`go:embed`)**:
   - File markdown otoritatif di root repositori dan direktori `docs/` (`README.md`, `README.id.md`, `docs/user-guide.md`, `docs/user-guide.id.md`, dan `CHANGELOG.md`) disematkan langsung ke dalam executable binary Pantau saat kompilasi (`main.go`).
...
2. **Batas Autentikasi Publik & Privat (Dual Entry Point)**:
   - Endpoint backend `GET /api/docs?name={readme|guide|changelog}&lang={en|id}` dirancang sebagai rute publik (tanpa token sesi) sehingga dapat diakses sebelum maupun setelah login.
   - Titik akses antarmuka disediakan di berbagai tempat: tautan bantuan di layar Login/Restore, tombol `(?)` di Header utama dashboard Pantau, klik badge nomor versi di footer, dan tautan riwayat versi di Settings Modal.
...
3. **Workspace Modal Container & Dynamic TOC**:
   ...
   - Bilah navigasi menyajikan pemilih dokumen (*README*, *User Guide*, dan *Changelog*) serta daftar isi dinamis (*Table of Contents*) berbasis heading (`h2`, `h3`) yang mendukung *smooth-scroll* langsung ke bagian dokumen atau versi rilis tertentu.
